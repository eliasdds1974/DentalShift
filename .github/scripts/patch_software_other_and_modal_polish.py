...
# Shared master list: Progident removed. "Other" is rendered separately where typing is supported.
...
patterns = [
    ...
    '<label className="field"><span>Software</span><select name="software" defaultValue="Any software"><option>Any software</option>{dentalSoftwareOptions.map((item) => <option key={item}>{item}</option>)}</select></label>',
...
for pattern in patterns:
    if pattern in text:
        text = text.replace(pattern, new_modal_software, 1)
        break
else:
    raise SystemExit('office modal software field pattern not found')

# Keep/add restrained DentalShift green polish to the modal.
