...
s = s.replace('type CalendarView = "month" | "week";', 'type CalendarView = "month" | "week" | "list";', 1)
s = s.replace('<div className="ml-1 grid grid-cols-2 rounded-xl bg-slate-100 p-1">\n            {(["month", "week"] as CalendarView[]).map', '<div className="ml-1 grid grid-cols-3 rounded-xl bg-slate-100 p-1">\n            {(["month", "week", "list"] as CalendarView[]).map', 1)

start = '      <div className="grid gap-0 xl:grid-cols-[minmax(0,1fr)_340px]">'
replacement = '''      {calendarView === "list" ? <div className="grid gap-4 p-4 sm:p-5 lg:grid-cols-2">
...
if start not in s:
    raise SystemExit('calendar grid start not found')
s = s.replace(start, replacement, 1)

end = '        </aside>\n      </div>\n    </section>'
if end not in s:
    raise SystemExit('calendar grid end not found')
s = s.replace(end, '        </aside>\n      </div>}\n    </section>', 1)
