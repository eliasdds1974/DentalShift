...
function ProfessionalCalendarWorkspace({ userId, profile, refreshKey, onNavigate }: { userId: string; profile: AccountProfile; refreshKey: number; onNavigate: (view: ProfessionalView) => void }) {
  ...
  const renderOpenShift = (shift: LiveShift) => {
    ...
    return <article key={shift.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        {application ? <Chip tone={application.status === "invited" ? "amber" : "blue"}>{application.status.replace("_", " ")}</Chip> : <button type="button" disabled={busy === `apply-${shift.id}`} onClick={() => void act(`apply-${shift.id}`, () => applyForShift({ shiftId: shift.id, professionalId: userId }))} className="primary-btn">{busy === `apply-${shift.id}` ? "Applying…" : "Apply"}</button>}
      </div>
    </article>;
  };
  ...
  return <div className="page-wrap">
    ...
    <section className="mt-6 grid gap-3 sm:grid-cols-3">
      <button type="button" onClick={() => setSelection({ type: "day" })} className="panel flex items-center gap-3 p-4 text-left"><span className={`grid h-10 w-10 place-items-center rounded-xl text-white ${signedRoleStyle.solid}`}><CalendarDays size={19} /></span><span><span className="block text-xs font-extrabold text-slate-500">Availability</span><strong className="block text-2xl text-[#002757]">{workflow.availability.filter((slot) => slot.available && new Date(slot.ends_at).getTime() >= Date.now()).length}</strong></span></button>
      <button type="button" onClick={() => onNavigate("bookings")} className="panel flex items-center gap-3 p-4 text-left"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#eaf8ee] text-[#017f27]"><Check size={19} /></span><span><span className="block text-xs font-extrabold text-slate-500">Booked</span><strong className="block text-2xl text-[#002757]">{upcomingBookings.length}</strong></span></button>
      <button type="button" onClick={() => onNavigate("shifts")} className="panel flex items-center gap-3 p-4 text-left"><span className="grid h-10 w-10 place-items-center rounded-xl bg-amber-50 text-amber-700"><FileCheck2 size={19} /></span><span><span className="block text-xs font-extrabold text-slate-500">Office requests</span><strong className="block text-2xl text-[#002757]">{officeRequests.length}</strong></span></button>
    </section>
      ...
      {calendarView === "list" ? <div className="grid gap-4 p-4 sm:p-5 lg:grid-cols-2">
        ...
        <div className="border-b border-slate-200 p-3 sm:p-5 lg:border-b-0 lg:border-r">
          ...
          <div className="grid grid-cols-7 gap-px overflow-hidden rounded-2xl border border-slate-200 bg-slate-200">{calendarDays.map((day) => {
            ...
            return <div key={key} className={`relative min-h-32 overflow-hidden bg-white p-2 transition sm:min-h-36 ${calendarView === "month" && !inMonth ? "text-slate-300" : "text-slate-800"} ${selected ? "z-10 ring-2 ring-inset ring-[#0078FE]" : ""}`}>
              <button type="button" aria-label={`Select ${key}`} onClick={() => chooseDate(day)} className="absolute inset-0 z-0 bg-transparent hover:bg-blue-50/40" />
              <div className="relative z-10 flex items-start justify-between gap-1 pointer-events-none">
                ...
                <div className="flex max-w-[72%] flex-wrap justify-end gap-1 pointer-events-auto">
              ...
              {firstAvailability && <button type="button" onClick={() => chooseItem(day, { type: "availability", id: firstAvailability.id })} className={`absolute inset-x-1.5 bottom-1.5 z-10 min-h-[46%] rounded-xl border p-2 text-left shadow-sm ${signedRoleStyle.soft} ${signedRoleStyle.border} ${signedRoleStyle.text}`}><span className="block text-[10px] font-black uppercase tracking-wide">Available</span><span className="mt-1 block text-[11px] font-extrabold leading-4">{shortTime(firstAvailability.starts_at)}–{shortTime(firstAvailability.ends_at)}</span>{dayAvailability.length > 1 && <span className="mt-1 block text-[10px] font-bold">+{dayAvailability.length - 1} more</span>}</button>}
            </div>;
          })}</div>
        </div>
        ...
        <aside className="bg-white p-4 sm:p-5">
          ...
          {selectedAvailability ? <div className={`rounded-2xl border p-4 ${signedRoleStyle.soft} ${signedRoleStyle.border}`}>
            ...
            <button type="button" disabled={busy === selectedAvailability.id} onClick={() => void act(selectedAvailability.id, () => removeProfessionalAvailability(selectedAvailability.id))} className="secondary-btn mt-4">{busy === selectedAvailability.id ? "Removing…" : "Remove availability"}</button>
            ...
            <section>
              ...
              {selectedDayAvailability.length ? <div className="mt-3 space-y-2">{selectedDayAvailability.map((slot) => <button key={slot.id} type="button" onClick={() => setSelection({ type: "availability", id: slot.id })} className={`w-full rounded-xl border p-3 text-left ${signedRoleStyle.soft} ${signedRoleStyle.border}`}><span className={`text-sm font-extrabold ${signedRoleStyle.text}`}>{shortTime(slot.starts_at)}–{shortTime(slot.ends_at)}</span></button>)}</div> : <form onSubmit={addAvailability} className="mt-3 rounded-2xl border-2 border-[#0078FE] bg-blue-50 p-4 shadow-md ring-4 ring-[#0078FE]/10"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[.1em] text-[#0064d8]">Availability needed</p><p className="mt-1 text-sm font-extrabold text-[#002757]">Set your hours for this date</p><p className="mt-1 text-xs leading-5 text-slate-600">Choose a start and end time so nearby offices can match you with shifts.</p></div><span className="mt-0.5 h-3 w-3 shrink-0 rounded-full bg-[#0078FE] ring-4 ring-[#0078FE]/15" /></div><div className="mt-3 grid grid-cols-2 gap-2"><label className="field"><span>Start</span><input name="start" type="time" step={900} defaultValue="08:00" required /></label><label className="field"><span>End</span><input name="end" type="time" step={900} defaultValue="16:30" required /></label></div><button type="submit" disabled={busy === "availability-add"} className="primary-btn mt-3 w-full justify-center">{busy === "availability-add" ? "Saving…" : "Add availability"}</button></form>}
            </section>
