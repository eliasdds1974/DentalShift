    ...
    <section className="mt-6 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      <div className="grid gap-3 border-b border-slate-200 p-3 sm:p-4">
        <div className="contents">
          <div className="flex min-w-0 flex-wrap items-baseline gap-x-3 gap-y-1"><h2 className="text-xl font-black tracking-tight text-[#002757] sm:text-2xl">{profession} calendar</h2><p className="text-sm font-bold leading-7 text-[#002757]">All open shifts posted by offices for your professional role are shown here.</p></div>
          ...
          <div className="grid grid-cols-7 gap-px overflow-hidden rounded-2xl border border-slate-200 bg-slate-200">{calendarDays.map((day) => {
            ...
            return <div key={key} className={`relative min-h-32 overflow-hidden bg-white p-2 transition sm:min-h-36 ${calendarView === "month" && !inMonth ? "text-slate-300" : "text-slate-800"} ${selected ? "z-10 ring-2 ring-inset ring-[#0078FE]" : ""}`}>
              ...
              <div className="relative z-10 pointer-events-none">
                ...
                {matchingOfficeRequests.length > 0 && <button type="button" onClick={() => chooseDate(day, true)} title={`${matchingOfficeRequests.length} office request${matchingOfficeRequests.length === 1 ? "" : "s"} for ${signedRoleStyle.label}`} className={`pointer-events-auto absolute inset-x-1.5 bottom-1.5 z-20 grid min-w-0 grid-cols-[minmax(0,1fr)_18px] items-center gap-1 rounded-lg bg-[#F21C13] px-1.5 py-1 text-white shadow-sm sm:grid-cols-[minmax(0,1fr)_20px]`}><span className="min-w-0 whitespace-nowrap text-center text-[8px] font-black leading-none tracking-[-0.03em] sm:text-[9px]">Office Request</span><span className="inline-grid h-[18px] w-[18px] place-items-center rounded-full bg-white text-[8px] font-black leading-none text-[#F21C13] sm:h-5 sm:w-5 sm:text-[9px]">{matchingOfficeRequests.length}</span></button>}
