          ...
          <div className="grid grid-cols-7 gap-px overflow-hidden rounded-2xl border border-slate-200 bg-slate-200">{calendarDays.map((day) => {
            ...
            return <div key={key} className={`relative min-h-32 overflow-hidden bg-white p-2 transition sm:min-h-36 ${calendarView === "month" && !inMonth ? "text-slate-300" : "text-slate-800"} ${selected ? "z-10 ring-2 ring-inset ring-[#0078FE]" : ""}`}>
              ...
              <div className="relative z-10 pointer-events-none">
                ...
                {matchingOfficeRequests.length > 0 && <button type="button" onClick={() => chooseDate(day)} title={`${matchingOfficeRequests.length} office request${matchingOfficeRequests.length === 1 ? "" : "s"} for ${signedRoleStyle.label}`} className="pointer-events-auto absolute left-0 right-0 top-7 flex min-w-0 items-center justify-between gap-1 rounded-lg bg-[#F21C13] px-1.5 py-1 text-[7px] font-black leading-none tracking-tight text-white shadow-sm sm:text-[8px]"><span className="min-w-0 whitespace-nowrap">Office Request</span><span className="inline-grid h-3.5 min-w-3.5 shrink-0 place-items-center rounded-full bg-white/25 px-1 text-[7px] sm:text-[8px]">{matchingOfficeRequests.length}</span></button>}
              ...
              {firstAvailability && <button type="button" onClick={() => chooseItem(day, { type: "availability", id: firstAvailability.id })} className="absolute inset-x-1.5 bottom-1.5 z-10 min-h-[46%] rounded-xl border border-[#04A62F]/35 bg-[#eaf8ee] p-2 text-left text-[#017f27] shadow-sm"><span className="block text-[10px] font-black tracking-wide">I’m Available 😊</span><span className="mt-1 block whitespace-nowrap text-[10px] font-extrabold leading-4">{shortTime(firstAvailability.starts_at)}–{shortTime(firstAvailability.ends_at)}</span>{dayAvailability.length > 1 && <span className="mt-1 block text-[10px] font-bold">+{dayAvailability.length - 1} more</span>}</button>}
            </div>;
          })}</div>
        </div>
        ...
        <aside className="bg-white p-4 sm:p-5">
          ...
          {selectedAvailability ? <div className="rounded-2xl border border-[#04A62F]/35 bg-[#eaf8ee] p-4">
            ...
            <section>
              ...
              {selectedDayAvailability.length ? <div className="mt-3 space-y-2">{selectedDayAvailability.map((slot) => <div key={slot.id} className="w-full rounded-xl border border-[#04A62F]/35 bg-[#eaf8ee] p-3"><span className="whitespace-nowrap text-sm font-extrabold text-[#017f27]">{shortTime(slot.starts_at)}–{shortTime(slot.ends_at)}</span>{editingAvailabilityId === slot.id ? <form onSubmit={(event) => void changeAvailability(event, slot)} className="mt-3 rounded-xl border border-[#0078FE]/25 bg-white p-3"><div className="grid grid-cols-2 gap-2"><label className="field"><span>Start</span><input name="start" type="time" step={900} defaultValue={new Date(slot.starts_at).toLocaleTimeString("en-CA", { hour: "2-digit", minute: "2-digit", hour12: false })} required /></label><label className="field"><span>End</span><input name="end" type="time" step={900} defaultValue={new Date(slot.ends_at).toLocaleTimeString("en-CA", { hour: "2-digit", minute: "2-digit", hour12: false })} required /></label></div><div className="mt-3 flex gap-2"><button type="submit" disabled={busy === `availability-change-${slot.id}`} className="primary-btn flex-1 justify-center">{busy === `availability-change-${slot.id}` ? "Saving…" : "Save time"}</button><button type="button" onClick={() => setEditingAvailabilityId(null)} className="secondary-btn">Cancel</button></div></form> : <div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={() => setEditingAvailabilityId(slot.id)} className="secondary-btn justify-center">Change time</button><button type="button" disabled={busy === slot.id} onClick={() => void act(slot.id, () => removeProfessionalAvailability(slot.id))} className="inline-flex min-h-11 items-center justify-center rounded-xl border-2 border-rose-500 bg-rose-50 px-3 py-2 text-sm font-extrabold text-rose-700">{busy === slot.id ? "Deleting…" : "Delete"}</button></div>}</div>)}</div> : <form onSubmit={addAvailability} className="hidden"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[.1em] text-[#0064d8]">Availability needed</p><p className="mt-1 text-sm font-extrabold text-[#002757]">Set your hours for this date</p><p className="mt-1 text-xs leading-5 text-slate-600">Choose a start and end time so nearby offices can match you with shifts.</p></div><span className="mt-0.5 h-3 w-3 shrink-0 rounded-full bg-[#04A62F] ring-4 ring-[#04A62F]/15" /></div><div className="mt-3 grid grid-cols-2 gap-2"><label className="field"><span>Start</span><input name="start" type="time" step={900} defaultValue="08:00" required /></label><label className="field"><span>End</span><input name="end" type="time" step={900} defaultValue="16:30" required /></label></div><button type="submit" disabled={busy === "availability-add"} className="primary-btn mt-3 w-full justify-center">{busy === "availability-add" ? "Saving…" : "Add availability"}</button></form>}
            </section>
