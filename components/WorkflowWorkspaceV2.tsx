...
function ProfessionalCalendarWorkspace({ userId, profile, refreshKey, onNavigate }: { userId: string; profile: AccountProfile; refreshKey: number; onNavigate: (view: ProfessionalView) => void }) {
  ...
  return <div className="page-wrap">
    ...
    <section className="mt-6 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      <div className="grid gap-4 border-b border-slate-200 p-4 sm:p-5 lg:grid-cols-[minmax(0,1fr)_170px]">
        <div className="contents">
          <div className="min-w-0 lg:col-span-2"><h2 className="text-2xl font-black tracking-tight text-[#002757]">Professional calendar</h2><p className="mt-1 text-sm text-slate-500">Availability fills the bottom half of each day. Bookings and office requests appear in the upper corner.</p></div>
          <div className="grid w-full gap-1.5 sm:w-[170px] lg:col-start-2 lg:row-start-2 lg:shrink-0"><button type="button" onClick={() => setSelection({ type: "day" })} className="flex h-9 w-full items-center gap-2 rounded-lg border border-[#0078FE]/20 bg-blue-50/60 px-2.5 text-left"><CalendarDays size={15} className="text-[#0078FE]" /><span><span className="block text-[10px] font-extrabold text-slate-500">Availability</span><strong className="ml-auto block text-sm leading-none text-[#002757]">{workflow.availability.filter((slot) => slot.available && new Date(slot.ends_at).getTime() >= Date.now()).length}</strong></span></button><button type="button" onClick={() => onNavigate("bookings")} className="flex h-9 w-full items-center gap-2 rounded-lg border border-[#01A32E]/20 bg-[#eaf8ee]/60 px-2.5 text-left"><Check size={15} className="text-[#017f27]" /><span><span className="block text-[10px] font-extrabold text-slate-500">Booked</span><strong className="ml-auto block text-sm leading-none text-[#002757]">{upcomingBookings.length}</strong></span></button><button type="button" onClick={() => onNavigate("shifts")} className="flex h-9 w-full items-center gap-2 rounded-lg border border-amber-200 bg-amber-50/70 px-2.5 text-left"><FileCheck2 size={15} className="text-amber-700" /><span><span className="block text-[10px] font-extrabold text-slate-500">Office requests</span><strong className="ml-auto block text-sm leading-none text-[#002757]">{officeRequests.length}</strong></span></button></div>
        </div>
