...
function isOfficePortal() {
  return window.localStorage.getItem("dentalshift_portal_role") === "office";
}
...
function identityFindings(text: string) {
...
function combinedOfficePostingText(form: HTMLFormElement) {
...
export function DentalJobsIdentityGuard() {
  ...
  useEffect(() => {
    const block = (event: Event, items: string[], title: string) => {
      event.preventDefault();
      event.stopPropagation();
      if ("stopImmediatePropagation" in event) event.stopImmediatePropagation();
      setWarning({ title, items });
    };

    const handleSubmit = (event: Event) => {
      if (!onDentalJobsPage() || !isOfficePortal()) return;
      if (!(event.target instanceof HTMLFormElement)) return;

      const form = event.target;
      if (!form.querySelector('textarea[name="description"]')) return;

      const items = identityFindings(combinedOfficePostingText(form));
      if (items.length > 0) {
        block(event, items, "This job ad cannot be posted yet");
      }
    };
    ...
    const handleClick = (event: Event) => {
      ...
      const items = identityFindings(importedText);
      if (items.length > 0) {
        block(event, items, "Remove identifying details before importing this ad");
      }
    };
