                    ...
                    {jobConnections.length === 0 ? (
                      ...
                      <div style={{ display: "grid", gap: 8, marginTop: 9 }}>
                        {jobConnections.map((item) => {
                          ...
                          const matchItem = { ...item, id: applicationId };
                              ...
                              <div style={{ flex: "0 1 290px", minWidth: 250, borderLeft: "1px solid #e2e8f0", padding: 10, display: "flex", flexDirection: "column", justifyContent: "center", gap: 7, background: "#fbfdff" }}>
                                {unlocked ? (
                                  ...
                                  <>
                                    ...
                                    <button disabled={!applicationId || unlockBusyId === applicationId} onClick={() => applicationId && onStartMatch(matchItem)} style={{ ...candidateActionBase, minHeight: 44, border: "1px solid #018d29", background: "linear-gradient(180deg,#10b63b 0%,#019c2f 100%)", color: "white", boxShadow: "0 5px 12px rgba(1,163,46,.22)", letterSpacing: ".02em", opacity: applicationId ? 1 : 0.55 }}><Handshake size={16} />{unlockBusyId === applicationId ? "Matching…" : "LET’S MATCH"}</button>
