  ...
  return (
    <section className="bg-[#f5f8fb] px-4 py-7 sm:px-6 lg:px-8 lg:py-9">
      <div className="mx-auto max-w-[1500px]">
        ...
        {loading ? (
          <div className="dentaljobs-listing-grid grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {[0, 1, 2, 3].map((item) => (
          ...
          <div className="dentaljobs-listing-grid grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{listings.map(renderCard)}</div>
        ) : (
                ...
                <div className="space-y-6">
                  {province.cities.map((city) => (
                    <section key={`${province.province}-${city.city}`} className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
                      ...
                      <div className="dentaljobs-listing-grid grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{city.listings.map(renderCard)}</div>
                    </section>
                  ))}
                </div>
              </section>
