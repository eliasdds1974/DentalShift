        ...
        <aside ref={resultsRef} className="scroll-mt-[92px] border-t border-slate-200 bg-white p-4 sm:p-5 lg:border-t-0 lg:p-0"><div className="lg:sticky lg:top-[84px] lg:max-h-[calc(100vh-100px)] lg:overflow-y-auto lg:p-5">
          ...
          <div className="space-y-4">
            ...
            {selectedShifts.length > 0 && <section className="rounded-3xl bg-[#04A62F] p-3 shadow-sm sm:p-4">
              <h3 className="mb-4 text-center text-xl font-black text-white sm:text-2xl">Posted Shifts</h3>
            ...
            {selectedBookings.length > 0 && <h3 className="pt-1 text-base font-black text-[#002757]">Booked</h3>}{selectedBookings.map((booking) => <article key={booking.id} className="rounded-2xl border border-[#04A62F]/25 bg-[#eaf8ee] p-4"><div className="flex items-center gap-2"><FileCheck2 size={17} className="text-[#04A62F]" /><strong className="text-[#032757]">BOOKED</strong></div><p className="mt-2 text-sm font-bold text-slate-700">{booking.contact?.name || "Confirmed professional"}</p>{booking.shifts && <p className="mt-1 text-xs text-slate-500">{booking.shifts.profession} · {shortTime(booking.shifts.starts_at)}–{shortTime(booking.shifts.ends_at)}</p>}</article>)}
          </div>
          </div>
        </aside>
      </div>}
