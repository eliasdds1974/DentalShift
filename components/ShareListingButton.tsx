...
export function ShareListingButton({ listingId, compact = false }: Props) {
  ...
  return <div className="relative">
    ...
    {open && <div className="absolute right-0 z-50 mt-2 w-64 overflow-hidden rounded-2xl border border-slate-200 bg-white p-2 shadow-2xl">
      ...
      <button type="button" onClick={shareFacebook} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-black text-[#1877F2] hover:bg-[#eef4ff]"><span className="grid h-[17px] w-[17px] place-items-center rounded-full bg-[#1877F2] text-[12px] font-black leading-none text-white">f</span> Share to Facebook</button>
