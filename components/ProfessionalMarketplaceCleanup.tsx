...
export function ProfessionalMarketplaceCleanup() {
  useEffect(() => {
    const tidyProfessionalMarketplace = () => {
      ...
      // Available shifts are already visible on the calendar. Keep only the
      // two status cards that tell the professional something the calendar
      // cannot: pending applications/invitations and confirmed bookings.
      ...
      const statusButtons = summaryButtons.filter((button) => button.textContent?.includes("Applications") || button.textContent?.includes("Confirmed bookings"));
      ...
      // One account represents one professional role. Keep only the calendar
      // view controls; profession matching is automatic.
