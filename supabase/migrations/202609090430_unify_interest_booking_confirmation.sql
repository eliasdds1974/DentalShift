...
begin
  ...
  if v_user = v_office_owner then
    if v_app.status <> 'applied'::public.application_status or v_app.application_kind <> 'application' then
      raise exception 'This professional interest is not available to book.';
    end if;
  elsif v_user = v_app.professional_id then
    if v_app.office_interested_at is null then raise exception 'This office has not expressed interest.'; end if;
    if v_app.status not in ('invited'::public.application_status, 'applied'::public.application_status) then
  ...
  if not exists(select 1 from public.professional_profiles where user_id = v_app.professional_id and licence_status = 'verified'::public.verification_status) then
  ...
  where s.id<>v_shift.id and s.interest_only=true and s.starts_at::date=v_shift.starts_at::date
    and exists(select 1 from public.applications a where a.shift_id=s.id and a.professional_id=v_app.professional_id and a.status='not_selected'::public.application_status);

  insert into public.booking_events(booking_id,actor_id,event_type) values(v_booking,v_user,'booking_confirmed');
...
grant execute on function public.confirm_interest_booking(uuid) to authenticated;
