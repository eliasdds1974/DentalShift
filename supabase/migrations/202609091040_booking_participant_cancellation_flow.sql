alter table public.bookings add column if not exists cancelled_by uuid null references public.profiles(id) on delete set null;
alter table public.bookings add column if not exists cancellation_party text null;
...
begin
  ...
  if coalesce(v_shift.interest_only,false) then
