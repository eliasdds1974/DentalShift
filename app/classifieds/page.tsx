...
export default function DentalJobsPage() {
  ...
  const [officeManageListing, setOfficeManageListing] = useState<OfficeJobListing | null>(null);
  ...
  const visibleConnections = portalRole === "professional"
  ...
  return <main className="min-h-screen bg-[#f5f8fb] text-slate-900">
    ...
    <section className="border-b border-[#002757]/10 bg-white">
      <div className="mx-auto max-w-7xl px-4 py-7 sm:px-6 lg:px-8">
        ...
        <div className={portalRole ? `mt-6 grid items-stretch gap-4 ${portalRole === "professional" ? "lg:grid-cols-2" : "lg:grid-cols-3"}` : ""}>
        ...
        {portalRole === "office" && <section id="my-dentaljobs" className="relative h-full min-h-[250px] min-w-0 scroll-mt-24 overflow-hidden rounded-2xl border-2 border-[#01A32E]/55 bg-[#effaf2] p-4 shadow-md sm:p-5"><div className="absolute inset-y-0 left-0 w-1.5 bg-[#01A32E]" /><div className="flex items-end justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.12em] text-[#017f27]">Office postings</p><h2 className="mt-1 text-xl font-black text-[#002757]">My DentalJobs</h2><p className="mt-1 text-sm text-slate-500">Manage your active and previous job ads.</p></div><span className="rounded-full border border-[#01A32E]/30 bg-white px-3 py-1.5 text-xs font-black text-[#017f27] shadow-sm">{myOfficeJobs.length} posting{myOfficeJobs.length === 1 ? "" : "s"}</span></div>{manageError && <p className="mt-3 rounded-xl bg-rose-50 p-3 text-sm font-bold text-rose-700">{manageError}</p>}<div className="mt-4 grid gap-3">{myOfficeJobs.length === 0 ? <div className="rounded-xl border border-dashed border-slate-300 bg-white p-5 text-sm font-semibold text-slate-500">You have no DentalJobs postings yet.</div> : myOfficeJobs.map((job) => { const daysLeft = Math.max(0, Math.ceil((new Date(job.expires_at).getTime() - Date.now()) / 86400000)); const isActive = job.status === "active" && daysLeft > 0; const displayStatus = job.status === "active" && daysLeft === 0 ? "expired" : job.status; const theme = getProfessionalCardTheme(job.profession); return <article key={job.id} className="min-h-[150px] rounded-2xl border-2 bg-white p-5 shadow-sm" style={{ borderColor: theme.border }}><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-[11px] font-black uppercase ${isActive ? "bg-[#eaf8ee] text-[#017f27]" : displayStatus === "paused" ? "bg-amber-50 text-amber-700" : displayStatus === "filled" ? "bg-blue-50 text-blue-700" : "bg-slate-100 text-slate-600"}`}>{displayStatus}</span>{isActive && <span className="text-xs font-bold text-slate-400">{daysLeft} day{daysLeft === 1 ? "" : "s"} remaining</span>}</div><h3 className="mt-2 font-black" style={{ color: theme.text }}>{job.profession} — {job.employment_type}</h3><p className="mt-1 text-sm text-slate-500">{job.city}, {job.province}</p><div className="mt-3"><ShareListingButton listingId={job.id} compact /></div></div><div className="flex w-full justify-end sm:w-auto">
          <button type="button" onClick={() => setOfficeManageListing(job)} className="inline-flex items-center justify-center gap-1 rounded-md bg-[#002757] px-2.5 py-1 text-[10px] font-black text-white shadow-sm transition hover:bg-[#01A32E]"><MoreVertical size={12}/> Manage</button>
        ...
        {professionalManageListing && (() => {
          const job = professionalManageListing;
          const daysLeft = Math.max(0, Math.ceil((new Date(job.expires_at).getTime() - Date.now()) / 86400000));
          const isActive = job.status === "active" && daysLeft > 0;
          const displayStatus = job.status === "active" && daysLeft === 0 ? "expired" : job.status;
          const theme = getProfessionalCardTheme(job.profession);
          return <div className="fixed inset-0 z-[80] flex items-center justify-center bg-[#001a38]/55 p-4" onMouseDown={(e) => { if (e.target === e.currentTarget) setProfessionalManageListing(null); }}>
            <div className="w-full max-w-sm overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl">
              <div className="flex items-start justify-between gap-4 border-b border-slate-100 p-5">
                <div><p className="text-xs font-black uppercase tracking-[0.12em]" style={{ color: theme.text }}>Manage</p><h3 className="mt-1 text-lg font-black text-[#002757]">{job.profession}</h3><p className="mt-1 text-sm font-semibold text-slate-500">{job.city}, {job.province}</p></div>
                <button type="button" onClick={() => setProfessionalManageListing(null)} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-slate-200 text-slate-500 hover:bg-slate-50"><X size={18}/></button>
              </div>
              <div className="grid grid-cols-2 gap-3 p-5">
                <button type="button" onClick={() => { setProfessionalManageListing(null); openEditProfessionalListing(job); }} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-black text-[#002757] hover:bg-[#edf3fa]"><Pencil size={16}/> Edit</button>
                {displayStatus === "paused" ? <button type="button" disabled={managingId === job.id} onClick={async () => { await manageProfessionalJob(job, "resume"); setProfessionalManageListing(null); }} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-black text-[#002757] hover:bg-[#edf3fa] disabled:opacity-50"><Play size={16}/> Resume</button> : <button type="button" disabled={!isActive || managingId === job.id} onClick={async () => { await manageProfessionalJob(job, "pause"); setProfessionalManageListing(null); }} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-black text-[#002757] hover:bg-[#edf3fa] disabled:opacity-50"><Pause size={16}/> Pause</button>}
                <button type="button" disabled={managingId === job.id} onClick={async () => { await manageProfessionalJob(job, "renew"); setProfessionalManageListing(null); }} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#01A32E] px-4 py-3 text-sm font-black text-white hover:bg-[#018a28] disabled:opacity-50"><RefreshCw size={16}/> Renew</button>
                <button type="button" disabled={managingId === job.id} onClick={async () => { await manageProfessionalJob(job, "delete"); setProfessionalManageListing(null); }} className="inline-flex items-center justify-center gap-2 rounded-xl border border-rose-200 bg-white px-4 py-3 text-sm font-black text-rose-600 hover:bg-rose-50 disabled:opacity-50"><Trash2 size={16}/> Delete</button>
              </div>
            </div>
          </div>;
        })()}

        {portalRole === "office" && <section className="h-full min-h-[250px] min-w-0 rounded-2xl border border-[#002757]/15 bg-white p-4 shadow-sm sm:p-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.12em] text-[#4285F4]">DentalJobs connections</p><h2 className="mt-1 text-xl font-black text-[#002757]">{portalRole === "office" ? "Dental Professionals Interest" : "Office Interest"}</h2><p className="mt-1 text-sm text-slate-500">{portalRole === "office" ? "Review dental professionals who applied or expressed interest in your DentalJobs postings." : "Review dental offices that expressed interest in your DentalJobs ad."}</p></div><span className="rounded-full bg-[#edf3fa] px-3 py-1.5 text-xs font-black text-[#002757]">{visibleConnections.length} connection{visibleConnections.length === 1 ? "" : "s"}</span></div>{connectionError && <p className="mt-3 rounded-xl bg-rose-50 p-3 text-sm font-bold text-rose-700">{connectionError}</p>}{unlockError && <div className="mt-3 flex flex-col gap-2 rounded-xl bg-rose-50 p-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-sm font-bold text-rose-700">{unlockError}</p>{unlockError.toLowerCase().includes("credit card") && <button type="button" onClick={() => void setupBillingCard()} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-[#002757] px-3.5 py-2 text-xs font-black text-white"><CreditCard size={14}/> Add Card</button>}</div>}<div className="mt-4 grid gap-3">{visibleConnections.length === 0 ? <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-5 text-sm font-semibold text-slate-500">No DentalJobs applications or expressions of interest yet.</div> : visibleConnections.map((item) => { const initiatedByMe = item.initiatorRole === portalRole; const statusLabel = item.status === "interested" ? "Interested" : item.status === "declined" ? "Not Interested" : item.status === "withdrawn" ? "Withdrawn" : "Pending"; return <article key={item.id} className={`relative rounded-2xl border border-slate-200 bg-white p-4 shadow-sm ${String(portalRole) === "professional" ? "pt-12" : ""}`}><button type="button" disabled={connectionDeletingId === item.id} onClick={() => void softDeleteConnection(item)} className="absolute right-3 top-3 inline-flex items-center gap-1 rounded-lg border border-rose-200 bg-white px-2.5 py-1.5 text-[11px] font-black text-rose-600 transition hover:bg-rose-50 disabled:opacity-50"><Trash2 size={12}/>{connectionDeletingId === item.id ? "Deleting…" : "Delete"}</button><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex flex-wrap gap-2"><span className={`rounded-full px-2.5 py-1 text-[11px] font-black uppercase ${item.status === "interested" ? "bg-[#eaf8ee] text-[#017f27]" : item.status === "declined" ? "bg-rose-50 text-rose-700" : item.status === "withdrawn" ? "bg-slate-100 text-slate-500" : "bg-amber-50 text-amber-700"}`}>{statusLabel}</span><span className="rounded-full bg-[#edf3fa] px-2.5 py-1 text-[11px] font-black text-[#002757]">{initiatedByMe ? "Sent by you" : "Received"}</span></div>{String(portalRole) === "professional" && item.initiatorRole === "office" && item.officeInterestSnapshot ? (() => { const office = item.officeInterestSnapshot; const min = office.pay_min == null ? null : Number(office.pay_min); const max = office.pay_max == null ? null : Number(office.pay_max); const pay = min != null || max != null ? `${min != null ? `$${min}` : ""}${min != null && max != null ? "–" : ""}${max != null ? `$${max}` : ""}/hr` : "Not specified"; return <div className="mt-3 max-w-2xl rounded-2xl border border-[#4285F4]/20 bg-[#f7faff] p-4"><div className="flex items-center gap-2"><Building2 size={17} className="text-[#002757]"/><p className="font-black text-[#002757]">Dental Office</p></div><div className="mt-3 grid gap-2 sm:grid-cols-2"><div className="rounded-xl bg-white p-3 ring-1 ring-slate-200"><p className="text-[10px] font-black uppercase tracking-wide text-slate-400">Location</p><p className="mt-1 text-sm font-bold text-[#002757]">{[office.city, office.province].filter(Boolean).join(", ") || "Not specified"}</p></div><div className="rounded-xl bg-white p-3 ring-1 ring-slate-200"><p className="text-[10px] font-black uppercase tracking-wide text-slate-400">Position</p><p className="mt-1 text-sm font-bold text-[#002757]">{office.profession || "Dental position"}</p></div><div className="rounded-xl bg-white p-3 ring-1 ring-slate-200"><p className="text-[10px] font-black uppercase tracking-wide text-slate-400">Employment</p><p className="mt-1 text-sm font-bold text-[#002757]">{office.employment_type || "Not specified"}</p></div><div className="rounded-xl bg-white p-3 ring-1 ring-slate-200"><p className="text-[10px] font-black uppercase tracking-wide text-slate-400">Approx. Days</p><p className="mt-1 text-sm font-bold text-[#002757]">{office.days_per_week ? `${office.days_per_week} day${String(office.days_per_week) === "1" ? "" : "s"} / week` : "Not specified"}</p></div><div className="rounded-xl bg-white p-3 ring-1 ring-slate-200"><p className="text-[10px] font-black uppercase tracking-wide text-slate-400">Pay</p><p className="mt-1 text-sm font-bold text-[#002757]">{pay}</p></div><div className="rounded-xl bg-white p-3 ring-1 ring-slate-200"><p className="text-[10px] font-black uppercase tracking-wide text-slate-400">Schedule / Hours</p><p className="mt-1 text-sm font-bold text-[#002757]">{office.schedule || "Not specified"}</p></div></div></div>; })() : <><h3 className="mt-2 font-black text-[#002757]">{item.profession}{item.employment ? ` — ${item.employment}` : ""}</h3><p className="mt-1 text-sm text-slate-500">{item.city}{item.city && item.province ? ", " : ""}{item.province}</p></>}{item.message && <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">“{item.message}”</p>}{portalRole === "office" && item.candidatePreview && <div className="mt-3 max-w-2xl rounded-2xl border border-[#4285F4]/20 bg-[#f7faff] p-4"><div className="flex items-center gap-2"><UserRound size={17} className="text-[#002757]"/><p className="font-black text-[#002757]">Dental Professional</p></div><div className="mt-3 grid gap-2 sm:grid-cols-2"><div className="rounded-xl bg-white p-3 ring-1 ring-slate-200"><p className="text-[10px] font-black uppercase tracking-wide text-slate-400">Location</p><p className="mt-1 text-sm font-bold text-[#002757]">{[item.candidatePreview.safeCity || item.city, item.candidatePreview.safeProvince || item.province].filter(Boolean).join(", ") || "Not specified"}</p></div><div className="rounded-xl bg-white p-3 ring-1 ring-slate-200"><p className="text-[10px] font-black uppercase tracking-wide text-slate-400">Profession</p><p className="mt-1 text-sm font-bold text-[#002757]">{item.candidatePreview.profession || item.profession || "Dental Professional"}</p></div><div className="rounded-xl bg-white p-3 ring-1 ring-slate-200"><p className="text-[10px] font-black uppercase tracking-wide text-slate-400">Employment</p><p className="mt-1 text-sm font-bold text-[#002757]">{item.employment || "Not specified"}</p></div><div className="rounded-xl bg-white p-3 ring-1 ring-slate-200"><p className="text-[10px] font-black uppercase tracking-wide text-slate-400">Experience</p><p className="mt-1 text-sm font-bold text-[#002757]">{item.candidatePreview.yearsExperience != null ? `${item.candidatePreview.yearsExperience} year${item.candidatePreview.yearsExperience === 1 ? "" : "s"}` : "Not specified"}</p></div></div>{item.candidatePreview.summary && <p className="mt-3 rounded-xl bg-white p-3 text-sm font-semibold leading-6 text-slate-600 ring-1 ring-slate-200">{item.candidatePreview.summary}</p>}{(item.candidatePreview.skills.length > 0 || item.candidatePreview.software.length > 0 || item.candidatePreview.certifications.length > 0) && <div className="mt-3 flex flex-wrap gap-2">{item.candidatePreview.skills.slice(0,4).map((skill) => <span key={skill} className="rounded-lg bg-white px-2.5 py-1.5 text-xs font-black text-slate-600 ring-1 ring-slate-200">{skill}</span>)}{item.candidatePreview.software.slice(0,3).map((software) => <span key={software} className="rounded-lg bg-[#eef4ff] px-2.5 py-1.5 text-xs font-black text-[#245FB8]">{software}</span>)}{item.candidatePreview.certifications.slice(0,3).map((certification) => <span key={certification} className="rounded-lg bg-[#eaf8ee] px-2.5 py-1.5 text-xs font-black text-[#017f27]">{certification}</span>)}</div>}<p className="mt-3 text-[11px] font-semibold leading-5 text-slate-400">Name, direct contact details, exact address, employer names and the original résumé remain private until candidate unlock.</p></div>}</div>{item.status === "pending" && <div className="flex flex-wrap gap-2 sm:justify-end">{String(portalRole) === "professional" ? (initiatedByMe ? <button type="button" disabled={connectionBusy} onClick={() => void updateConnection(item,"withdrawn")} className="rounded-xl border border-slate-300 bg-white px-3.5 py-2 text-xs font-black text-slate-600 hover:bg-slate-50 disabled:opacity-50">Withdraw</button> : <><button type="button" disabled={connectionBusy} onClick={() => void updateConnection(item,"declined")} className="rounded-xl border border-rose-200 bg-white px-3.5 py-2 text-xs font-black text-rose-600 hover:bg-rose-50 disabled:opacity-50">Not Interested</button><button type="button" disabled={connectionBusy} onClick={() => void updateConnection(item,"interested")} className="rounded-xl bg-[#01A32E] px-3.5 py-2 text-xs font-black text-white shadow-sm hover:bg-[#018a28] disabled:opacity-50">Interested</button></>) : initiatedByMe ? <span className="rounded-xl bg-amber-50 px-3.5 py-2 text-xs font-black text-amber-700">Awaiting Response</span> : <><button type="button" disabled={connectionBusy || unlockBusyId === item.id} onClick={() => void updateConnection(item,"declined")} className="rounded-xl border border-rose-200 bg-white px-3.5 py-2 text-xs font-black text-rose-600 hover:bg-rose-50 disabled:opacity-50">Not Interested</button><button type="button" disabled={unlockBusyId === item.id} onClick={() => void startCandidateUnlock(item)} className="inline-flex items-center gap-2 rounded-xl bg-[#01A32E] px-4 py-2.5 text-xs font-black text-white shadow-md hover:bg-[#018a28] disabled:opacity-50"><CreditCard size={15}/>{unlockBusyId === item.id ? "Unlocking…" : "Unlock Candidate — $29 CAD · billed monthly"}</button></>}</div>}{item.status === "interested" && <div className="flex flex-wrap gap-2 sm:justify-end">{isCandidateUnlocked(item) ? <>{portalRole === "office" && <button type="button" disabled={unlockBusyId === item.id} onClick={() => void viewUnlockedCandidate(item)} className="inline-flex items-center gap-2 rounded-xl border border-[#01A32E]/30 bg-[#eaf8ee] px-4 py-2.5 text-xs font-black text-[#017f27] hover:bg-[#dff5e5]"><FileText size={15}/> View Candidate</button>}<button type="button" onClick={() => void openChat(item)} className="inline-flex items-center gap-2 rounded-xl bg-[#002757] px-4 py-2.5 text-xs font-black text-white shadow-md transition hover:-translate-y-0.5 hover:bg-[#01A32E] hover:shadow-lg"><MessageCircle size={15}/> Message</button></> : portalRole === "office" ? <button type="button" disabled={unlockBusyId === item.id} onClick={() => void startCandidateUnlock(item)} className="inline-flex items-center gap-2 rounded-xl bg-[#01A32E] px-4 py-2.5 text-xs font-black text-white shadow-md hover:bg-[#018a28] disabled:opacity-50"><CreditCard size={15}/>{unlockBusyId === item.id ? "Unlocking…" : "Unlock Candidate — $29 CAD · billed monthly"}</button> : <span className="rounded-xl bg-amber-50 px-3.5 py-2 text-xs font-black text-amber-700">Waiting for office to unlock connection</span>}</div>}</div></article>})}</div></section>}
