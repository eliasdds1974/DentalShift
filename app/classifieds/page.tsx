  ...
  return <main className="min-h-screen bg-[#f5f8fb] text-slate-900">
    ...
    <section className="border-b border-[#002757]/10 bg-white">
      <div className="mx-auto max-w-7xl px-4 py-7 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 grid gap-3 rounded-2xl border-2 border-[#002757] bg-[#f8fafc] p-3 shadow-sm md:grid-cols-[1.4fr_.8fr_.9fr] lg:grid-cols-[1.5fr_.7fr_.9fr_auto]">
    ...
    <section className="mx-auto max-w-7xl px-4 py-7 sm:px-6 lg:px-8">
      ...
      <div className="grid gap-5 lg:grid-cols-2">{visibleAds.map((ad) => <article key={ad.id} className={`group relative overflow-hidden rounded-3xl border-2 bg-white shadow-md transition duration-200 hover:-translate-y-1 hover:shadow-xl ${ad.kind === "office" ? "border-[#002757]/18" : "border-[#01A32E]/30"} ${ad.featured ? "ring-2 ring-[#FDB605]/30" : ""}`}><div className={`absolute inset-x-0 top-0 h-1.5 ${ad.kind === "office" ? "bg-[#002757]" : "bg-[#01A32E]"}`} />{ad.featured && <div className="absolute right-4 top-4 rounded-full bg-[#FDB605] px-3 py-1.5 text-[11px] font-black text-white shadow-sm"><Star size={12} className="mr-1 inline fill-white" />Featured</div>}<div className="p-5 sm:p-6"><div className="flex items-start gap-4"><div className={`grid h-14 w-14 shrink-0 place-items-center rounded-2xl shadow-sm ${ad.kind === "office" ? "bg-[#002757] text-white" : "bg-[#01A32E] text-white"}`}>{ad.kind === "office" ? <Building2 size={26} /> : <UserRound size={26} />}</div><div className="min-w-0 flex-1 pr-16"><span className={`inline-flex rounded-full px-3 py-1 text-[11px] font-black tracking-wide ${ad.kind === "office" ? "bg-[#edf3fa] text-[#002757]" : "bg-[#eaf8ee] text-[#017f27]"}`}>{ad.kind === "office" ? "OFFICE HIRING" : "PROFESSIONAL SEEKING OFFICE"}</span><h3 className={`mt-2 text-xl font-black leading-6 ${ad.kind === "office" ? "text-[#002757]" : "text-[#017f27]"}`}>{ad.title}</h3></div></div><div className="mt-5 grid gap-2.5 sm:grid-cols-2"><div className="rounded-xl bg-slate-50 px-3.5 py-3"><p className="text-[10px] font-black uppercase tracking-[0.12em] text-slate-400">Location</p><p className="mt-1 inline-flex items-center gap-1.5 text-sm font-bold text-slate-700"><MapPin size={15} />{ad.city}</p></div><div className="rounded-xl bg-slate-50 px-3.5 py-3"><p className="text-[10px] font-black uppercase tracking-[0.12em] text-slate-400">Distance</p><p className="mt-1 text-sm font-bold text-slate-700">{ad.distance} km away</p></div></div><p className="mt-4 line-clamp-3 text-sm leading-6 text-slate-600">{ad.description}</p><div className="mt-5 flex flex-wrap gap-2"><span className={`rounded-lg px-3 py-2 text-xs font-black ${ad.kind === "office" ? "bg-[#edf3fa] text-[#002757]" : "bg-[#eaf8ee] text-[#017f27]"}`}>{ad.employment}</span><span className="rounded-lg bg-[#fff7df] px-3 py-2 text-xs font-black text-[#8a6200]">{ad.pay}</span><span className="rounded-lg bg-slate-100 px-3 py-2 text-xs font-bold text-slate-500"><Clock3 size={13} className="mr-1 inline" />{ad.posted}</span></div></div><div className={`flex items-center justify-between gap-3 border-t px-5 py-4 sm:px-6 ${ad.kind === "office" ? "border-[#002757]/10 bg-[#f7f9fc]" : "border-[#01A32E]/15 bg-[#f5fbf6]"}`}><span className={`text-xs font-black ${ad.kind === "office" ? "text-[#002757]" : "text-[#017f27]"}`}>{ad.kind === "office" ? "Anonymous office opportunity" : "Anonymous professional profile"}</span><button type="button" className={`rounded-xl px-4 py-2.5 text-sm font-black text-white shadow-md transition hover:-translate-y-0.5 hover:shadow-lg ${ad.kind === "office" ? "bg-[#002757] hover:bg-[#01A32E]" : "bg-[#01A32E] hover:bg-[#002757]"}`}>View Opportunity</button></div></article>)}</div>
    </section>
