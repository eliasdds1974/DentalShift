...
export async function POST(request: Request) {
  ...
  const { data: unlock } = await admin.from("candidate_unlocks").select("status,paid_at").eq("office_id", application.office_id).eq("professional_id", application.professional_id).eq("status", "paid").maybeSingle();
  ...
  return NextResponse.json({
    unlocked: true,
    unlockedAt: unlock.paid_at,
    candidate: {
      name: name || "Dental Professional",
      ...
      licenceNumber: professional?.licence_number ?? null,
      licenceProvince: professional?.licence_province ?? null,
      resumeUrl,
    },
  });
}
