...
export async function POST(request: Request) {
  ...
  const { data: billing } = await admin.from("office_billing_profiles").select("billing_status,stripe_payment_method_id").eq("office_id", application.office_id).maybeSingle();
  if (!billing || billing.billing_status !== "active" || !billing.stripe_payment_method_id) {
