...
async function fallbackLoad(userId: string): Promise<AccountDetails> {
  ...
  if (profileResult.error || !profileResult.data) {
    throw profileResult.error || new Error("Your DentalShift profile could not be loaded.");
  }

  const profile = profileResult.data as AccountProfile;

  const [professionalResult, officeResult] = await Promise.all([
    withTimeout(
      supabase
  ...
  if (professional?.licence_status === "needs_review") {
    try {
      const decisionResult = await withTimeout(
        supabase
      ...
      if (decisionResult.data?.notes) {
        verificationRequest = {
          notes: decisionResult.data.notes,
          created_at: decisionResult.data.created_at,
        };
      }
    } catch {
...
export async function loadAccountDetails(userId: string): Promise<AccountDetails> {
  try {
    return await withTimeout(core.loadAccountDetails(userId), 6500, "Account request");
  } catch {
    return await fallbackLoad(userId);
  }
}
